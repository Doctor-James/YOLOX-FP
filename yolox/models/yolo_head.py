...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                output = torch.cat([reg_output, points_output, obj_output, cls_output], 1)
                output, grid = self.get_output_and_grid(
                    output, k, stride_this_level, xin[0].type()
                )
                x_shifts.append(grid[:, :, 0])
                y_shifts.append(grid[:, :, 1]) # 得到每个grid的xy坐标
                ...
                output = torch.cat(
                    [reg_output, points_output, obj_output.sigmoid(), cls_output.sigmoid()], 1
                )
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        n_ch = 5 + 8 +  self.num_classes
        hsize, wsize = output.shape[-2:]
        if grid.shape[2:4] != output.shape[2:4]:
            yv, xv = meshgrid([torch.arange(hsize), torch.arange(wsize)])
            grid = torch.stack((xv, yv), 2).view(1, 1, hsize, wsize, 2).type(dtype)
            self.grids[k] = grid #生成网格坐标 [1,1,80,80,2],最后一个2是指每个grid的（x,y）

        output = output.view(batch_size, self.n_anchors, n_ch, hsize, wsize) # self.n_anchors = 1
        output = output.permute(0, 1, 3, 4, 2).reshape(
            batch_size, self.n_anchors * hsize * wsize, -1
        )# output = [batch_size,self.n_anchors * hsize * wsize,n_ch]
    ...
    def get_losses(
        ...
        y_shifts,
        expanded_strides,
        labels, # [batchsize,120,5+8] 120:YOLOX规定每张图片最多120个标签，不足120补0
        outputs,
        origin_preds,
        dtype,
    ):
        bbox_preds = outputs[:, :, :4]  # [batch, n_anchors_all = 80*80+40*40+20*20, 4]
        pit_preds = outputs[:, :, 4:12] # [batch, n_anchors_all = 80*80+40*40+20*20, 8]
        obj_preds = outputs[:, :, 12].unsqueeze(-1)  # [batch, n_anchors_all, 1]
        cls_preds = outputs[:, :, 13:]  # [batch, n_anchors_all, n_cls]

        # calculate targets
        nlabel = (labels.sum(dim=2) > 0).sum(dim=1)  # number of objects [第一张图片的目标数，第二张图片的目标数,...]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_bboxes_per_image = labels[batch_idx, :num_gt, 1:5] #labels中的bbox值
                gt_classes = labels[batch_idx, :num_gt, 0] #labels中的class值
